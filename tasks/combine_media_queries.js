...
module.exports = function (grunt) {
    grunt.registerMultiTask('cmq', 'Find duplicate media queries and combines them.', function () {
        ...
        var options = this.options({
            log: false,
            ext: false,
            splitWithSuffix: false
        });

        var splitWithSuffix = options.splitWithSuffix;
        ...
        this.files.forEach(function (f) {
            ...
            f.src.forEach(function (filepath) {
                ...
                if (splitWithSuffix !== false && typeof(splitWithSuffix) === 'string') {
                    var matches = /(.*)(\.css)/.exec(filename);
                    mqFilename = matches[1] + '.' + splitWithSuffix + matches[2];
                    mqDestPath = path.join(f.dest, mqFilename);
                    splitWithSuffix = true;
                }
                ...
                if (options.ext) {
                    ...
                    mqDestPath = destpath.replace(/\.(.*)/, options.ext);
                }

                // Normalize line endings
                strStyles = grunt.util.normalizelf(strStyles);
                if (splitWithSuffix) {
                    strMQStyles = grunt.util.normalizelf(strMQStyles);
                    grunt.file.write(mqDestPath, strMQStyles);
                }
